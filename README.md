...
`uvx` **不是协议，也不是 PPTLint 自带的命令**。它是 [Astral uv](https://docs.astral.sh/uv/guides/tools/) 提供的 Python 工具运行器：第一次运行时联网下载 PPTLint，并放在临时、隔离的环境中执行，避免污染你现有的 Python 环境。没有安装 `uv` 的电脑也不会有 `uvx`。

你可以先运行 `uvx --version` 检查。没有这个命令时，任选一种方式：

**方式 A：安装 uv 后使用 `uvx`（推荐，后续无需单独管理 PPTLint 环境）**
...
Windows、Linux 或没有 Homebrew 的 macOS，请按 [uv 官方安装说明](https://docs.astral.sh/uv/getting-started/installation/) 安装。完成后重新打开终端，再运行上面的 `uvx pptlint ...`。

**方式 B：不安装 uv，使用已有的 Python 3.10+**
...
pptlint check output.pptx --scenario present --lang zh-CN
```
...
采用方式 B 后，本文后续命令中的 `uvx pptlint` 直接替换为 `pptlint`，`uvx --refresh pptlint app` 则替换为 `pptlint app`。
...
它会在随机的 `127.0.0.1` 端口打开中文页面。检查后，任务会按风险和页面整理，每项问题同时给出 PowerPoint 桌面版人工步骤；WPS 的菜单名称和入口位置可能不同，应按同一对象和目标操作。真实旧稿的视觉修改目前坚持人工优先：在原生、保真的对象级编辑链路完成前，不再把整份 PPT 导入后重新导出来冒充“一键局部优化”，避免母版、透明背景、组合对象、链接和未选页被破坏。
