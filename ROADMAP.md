...
- `pptlint-repair-plan/v1` 覆盖全部问题，为每项任务提供稳定 ID、位置、风险、执行器和验收条件。
- 中央修复目录覆盖所有内置规则；未知规则默认需要人工判断。
- 为通用 Agent、Ultimate PPT Master、PowerPoint Copilot 和手工 PowerPoint 生成适配简报。
- GitHub Action 上传机器可读修复计划，HTML 报告显示每项问题的安全处理路径。

## v1.2 — 显式隐私清理副本

- 只在用户逐项授权时清理作者信息、批注和讲者备注。
- 永不覆盖原文件，生成修改回执、修改前后报告和任务验证结果。

## v1.3 — 本地拖放式应用

- 只绑定 `127.0.0.1` 随机端口，不请求外部字体、统计、模型或接口。
- 支持检查、授权清理副本、复制 Agent 任务、修改后复检和导出 Proof Pack。
