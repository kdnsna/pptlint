...
function taskMarkup(t){const loc=t.slideIndex?`第 ${t.slideIndex} 页`:'整个文件';return `<details class="task"><summary><span></span><span class="task-title"><b>${esc(t.consequence)}</b><small>${esc(modeLabels[t.repairMode]||t.repairMode)} · 风险 ${esc(t.risk)}</small></span></summary><div class="task-body"><b>在哪里</b><p>${loc}，详细对象位置可在中文检查报告中查看。</p><b>希望改成什么样</b><p>${esc(t.target)}</p><b>自己在 PowerPoint 里这样改</b><ol>${(t.steps||[]).map(step=>`<li>${esc(step)}</li>`).join('')}</ol><b>改完这样检查</b><p>以 100% 比例或放映模式查看修改页，再把独立副本拖入第三步复检。</p><div class="task-route"><span><strong>${esc(modeLabels[t.repairMode]||t.repairMode)}</strong><small>${esc(routeDetails[t.repairMode]||'按完整报告中的处理方式继续。')}</small></span></div></div></details>`}
function groupTasks(tasks){const grouped=new Map();tasks.forEach(task=>{const key=task.slideIndex?`slide-${task.slideIndex}`:'whole-file';if(!grouped.has(key))grouped.set(key,{slideIndex:task.slideIndex||null,tasks:[]});grouped.get(key).tasks.push(task)});return [...grouped.values()].map(group=>{group.tasks.sort((a,b)=>(riskRanks[a.risk]??9)-(riskRanks[b.risk]??9)||String(a.ruleId).localeCompare(String(b.ruleId)));group.risk=Math.min(...group.tasks.map(task=>riskRanks[task.risk]??9));return group}).sort((a,b)=>a.risk-b.risk||(a.slideIndex===null?1:b.slideIndex===null?-1:a.slideIndex-b.slideIndex))}
function pageGroupMarkup(group){const loc=group.slideIndex?`第 ${group.slideIndex} 页`:'整个文件';return `<section class="page-group"><div class="page-head"><strong>${loc}</strong><span>${group.tasks.length} 项 · 优先看高风险任务</span></div>${group.tasks.map(taskMarkup).join('')}</section>`}
function renderCheck(d,restored=false){const r=d.readiness||{},cls=r.status||'review',label={ready:'可以交付',review:'发送前再看一眼',blocked:'先处理再发送'}[cls]||'需要检查',tasks=d.tasks||[],actionPages=new Set(tasks.filter(t=>Number.isInteger(t.slideIndex)&&(t.steps||[]).length).map(t=>t.slideIndex)).size,cleanupCount=(d.cleanupOperations||[]).length,humanCount=d.modeCounts['human-decision']||0,groups=groupTasks(tasks),fileName=d.file&&d.file.name?d.file.name:'当前 PPT',recovery=restored?'<div style="margin:0 0 10px;padding:9px 11px;border-left:3px solid var(--green);background:#edf7f3;color:var(--muted);font-size:12px">已恢复当前标签页的本地会话。这里只记住会话编号；原 PPT 仍只存在于本次运行的临时目录，关闭交付台后删除。</div>':'';document.getElementById('check-result').innerHTML=`${recovery}<div class="status ${cls}"><div><b>${label}</b><div style="overflow-wrap:anywhere">${esc(fileName)} · ${tasks.length} 项任务，已按风险和页面整理；辅助分数请查看完整报告。</div></div></div><div class="metrics"><div class="metric"><span>待处理页面</span><strong>${actionPages}</strong></div><div class="metric"><span>可安全清理</span><strong>${cleanupCount}</strong></div><div class="metric"><span>需要判断</span><strong>${humanCount}</strong></div></div><div class="tasks">${groups.length?groups.map(pageGroupMarkup).join(''):'<div class="status"><b>没有需要处理的任务</b></div>'}</div>${downloads(d.downloads)}`;document.getElementById('check-result').classList.add('show')}
