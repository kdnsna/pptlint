...
<style>
:root{--ink:#0a1628;--muted:#657083;--paper:#f4f1eb;--panel:#fff;--accent:#e85d2c;--green:#0d8b63;--amber:#b66a09;--red:#c3382b;--line:#dfe2e7}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.65 "PingFang SC","Microsoft YaHei","Noto Sans CJK SC",sans-serif}button,input,select{font:inherit}.shell{width:min(1240px,calc(100% - 32px));margin:auto}.nav{display:flex;justify-content:space-between;align-items:center;padding:20px 0}.brand{font:900 17px/1 "SFMono-Regular",Menlo,Monaco,Consolas,monospace;letter-spacing:.14em}.privacy{color:var(--muted);font-size:13px}.hero{display:grid;grid-template-columns:1.2fr .8fr;gap:28px;padding:52px 0 36px}.eyebrow{font:800 12px/1 "SFMono-Regular",Menlo,Monaco,Consolas,monospace;letter-spacing:.13em;color:var(--accent)}h1{font-size:clamp(42px,6.2vw,72px);line-height:.98;letter-spacing:-.055em;margin:14px 0 22px}.lead{font-size:18px;color:var(--muted);max-width:720px}.trust{align-self:end;background:var(--ink);color:white;border-radius:24px;padding:26px;box-shadow:0 24px 70px #0a16282a}.trust strong{display:block;font-size:28px}.trust span{display:block;color:#c6cfdd;margin-top:8px}.steps{display:grid;grid-template-columns:minmax(360px,1.35fr) minmax(320px,.95fr) minmax(280px,.9fr);gap:16px;margin:18px 0 70px;align-items:start}.card{min-width:0;background:var(--panel);border:1px solid var(--line);border-radius:20px;padding:24px;box-shadow:0 14px 40px #0a16280c}.card h2{font-size:24px;margin:8px 0}.card>p{color:var(--muted)}.num{font:800 12px "SFMono-Regular",Menlo,Monaco,Consolas,monospace;color:var(--accent)}.drop{display:flex;flex-direction:column;align-items:center;justify-content:center;width:100%;min-height:136px;border:2px dashed #aeb6c2;border-radius:16px;padding:28px 18px;text-align:center;background:#fafbfc;color:var(--ink);cursor:pointer;transition:.2s}.drop.drag{border-color:var(--accent);background:#fff4ee}.drop input{display:none}.drop b{display:block;width:100%;color:var(--ink);font-size:18px}.drop span{display:block;width:100%;color:var(--muted);font-size:13px}.select{display:block;width:100%;border:1px solid #ccd2da;border-radius:10px;padding:10px 12px;margin:14px 0;background:white}.result{display:none;margin-top:22px}.result.show{display:block}.status{display:flex;gap:12px;align-items:center;padding:16px;border-radius:14px;background:#edf7f3}.status.blocked{background:#fff0ed}.status.review{background:#fff6e8}.status b{font-size:20px}.metrics{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:12px 0}.metric{background:#f4f6f8;padding:12px;border-radius:10px}.metric strong{display:block;font-size:22px}.tasks{max-height:580px;overflow:auto;border-top:1px solid #e2e5e9}.page-group{border-bottom:1px solid #cfd5dd;padding:12px 0 4px}.page-head{display:flex;justify-content:space-between;gap:12px;align-items:baseline;margin-bottom:2px}.page-head strong{font-size:16px}.page-head span{color:var(--muted);font-size:12px}.task{border-top:1px solid #edf0f3}.task summary{display:grid;grid-template-columns:auto 1fr;gap:10px;padding:12px 2px;cursor:pointer;list-style:none}.task summary::-webkit-details-marker{display:none}.task summary:before{content:"+";display:grid;place-items:center;width:22px;height:22px;border-radius:50%;background:#eef1f5;font-weight:900}.task[open] summary:before{content:"−"}.task-title b{display:block}.task-title small{color:var(--muted)}.task-body{padding:0 2px 16px 32px}.task-body p{margin:4px 0 8px}.task-body ol{margin:6px 0;padding-left:20px;color:var(--muted)}.task-route{display:flex;align-items:flex-start;gap:8px;background:#f5f6f8;border-radius:10px;padding:9px 10px;margin-top:10px}.task-route strong,.task-route small{display:block}.task-route small{color:var(--muted)}.choice{border:1px solid var(--line);border-radius:14px;padding:14px;margin:12px 0}.choice strong{display:block;font-size:17px}.choice p{margin:4px 0;color:var(--muted)}.choice.recommended{border-color:#8ebba9;background:#f2f8f5}.choice-tag{font:800 10px "SFMono-Regular",Menlo,Monaco,Consolas,monospace;color:var(--green)}.actions{display:flex;flex-wrap:wrap;gap:9px;margin-top:12px}.btn{border:0;border-radius:10px;padding:11px 14px;font-weight:750;cursor:pointer;background:var(--ink);color:white;text-decoration:none}.btn.alt{background:white;color:var(--ink);border:1px solid #b8c0cb}.btn:disabled{opacity:.38;cursor:not-allowed}.checks{display:grid;gap:8px;margin:10px 0}.checks label{display:flex;gap:8px;align-items:center;background:#f5f6f8;padding:9px 10px;border-radius:9px}.subhead{margin:20px 0 6px;font-size:13px}.message{margin-top:12px;padding:12px;border-left:4px solid var(--green);background:#edf7f3;display:none;overflow-wrap:anywhere}.message.show{display:block}.message.error{border-color:var(--red);background:#fff0ed}.message code{display:block;margin-top:8px;padding:8px;background:white;white-space:pre-wrap}.downloads{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}.downloads a{color:#1457a6}.platform-note{font-size:12px;color:var(--muted);margin-top:10px}.busy-line{display:grid;gap:5px;padding:16px;border-left:4px solid var(--accent);background:#fff6e8}.busy-line strong{overflow-wrap:anywhere}.busy-line span{color:var(--muted);font-size:13px}.footer{padding:24px 0 54px;color:var(--muted);font-size:13px}.spinner .drop{opacity:.55;pointer-events:none}@media(max-width:1000px){.steps{grid-template-columns:1fr 1fr}.steps .card:nth-child(2){grid-column:span 1}.steps .card:nth-child(3){grid-column:1/-1}}@media(max-width:760px){.hero,.steps{grid-template-columns:1fr}.steps .card:nth-child(3){grid-column:auto}.trust{align-self:auto}}@media(max-width:430px){.shell{width:min(100% - 22px,1240px)}.nav{align-items:flex-start;gap:10px}.brand{font-size:14px;white-space:nowrap}.privacy{max-width:160px;text-align:right}.hero{padding-top:28px}h1{font-size:40px}.card{padding:18px}.metrics{grid-template-columns:1fr}.actions{display:grid}.btn{text-align:center;width:100%}.page-head{display:block}}
</style></head><body><div class="shell">
<nav class="nav"><div class="brand">PPTLINT / LOCAL</div><div class="privacy">只运行在本机 · 不上传 · 不调用模型</div></nav>
<header class="hero"><div><span class="eyebrow">POWERPOINT 发出前的最后一关</span><h1>找到难改的页面，<br>告诉你怎么改。</h1><p class="lead">先定位影响阅读和交付的问题，再按命中页面给出的步骤处理。PPTLint 不会把整份真实 PPT 重导出来冒充局部优化，也不会把高风险修改包装成一键完成。</p></div><aside class="trust"><strong>先说清楚，再动文件</strong><span>源文件永远不动。只有不会重排画面的隐私清理可以生成新副本；其余问题提供人工步骤和复检证据。</span></aside></header>
<main class="steps">
...
<section class="card"><span class="num">02 · 选择怎么改</span><h2>优先按命中页处理</h2><div class="choice recommended"><span class="choice-tag">推荐 · 最稳妥</span><strong>A · 自己按步骤调整</strong><p>展开第一栏的页面任务，依次查看位置、目标、人工步骤和改后检查，不重建整份文件。</p><div class="platform-note">菜单步骤以 PowerPoint 桌面版为主；WPS 的入口名称和位置可能不同，请按相同对象与目标操作。</div></div><div class="choice"><strong>B · 复制给助手继续分析</strong><p>复制完整任务用于获得更细建议。助手处理的仍是独立副本，PPTLint 当前不会自动修改布局、表格或截图流程。</p><div class="actions"><button class="btn alt" id="copy-btn" disabled>复制完整任务</button></div></div><h3 class="subhead">PPTLint 可安全处理的隐私内容</h3><div class="checks" id="cleanup-checks"><span style="color:var(--muted)">先完成第一步。</span></div><button class="btn alt" id="cleanup-btn" disabled>生成清理副本</button><div class="message" id="action-message" aria-live="polite"></div></section>
...
<script>
const TOKEN=__TOKEN__;const MAX_UPLOAD_BYTES=__MAX_UPLOAD_BYTES__;const SESSION_KEY='pptlint.currentDeckId';const state={deck:null};
const opLabels={'clear-personal-metadata':'清除作者和最后编辑者信息','remove-comments':'移除批注','remove-speaker-notes':'移除讲者备注'};
const modeLabels={'cleanup-copy':'PPTLint 可生成安全清理副本','guided-powerpoint':'只能按步骤人工调整','agent-rebuild':'高风险，不建议自动处理','human-decision':'需要你先判断'};
const routeDetails={'cleanup-copy':'只有你明确勾选后，PPTLint 才会处理新副本。','guided-powerpoint':'PPTLint 提供定位、调整步骤和复检，不会替你改文件。','agent-rebuild':'涉及布局或语义判断；先人工重排或让助手分析，PPTLint 不改文件。','human-decision':'先确认是否应该改或删，再选择人工处理方式。'};
const riskRanks={high:0,medium:1,low:2};
const riskLabels={high:'高风险',medium:'中风险',low:'低风险'};
function auth(){return{'X-PPTLint-Token':TOKEN}}
function esc(s){return String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function downloads(items){return '<div class="downloads">'+(items||[]).map(x=>`<a href="${esc(x.url)}" target="_blank" rel="noopener" title="在新标签页打开，不离开本地交付台">${esc(x.label)}</a>`).join('')+'</div>'}
function wireDrop(id,fn){const box=document.getElementById(id),input=box.querySelector('input');box.addEventListener('click',()=>input.click());box.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();input.click()}});input.addEventListener('change',()=>{if(input.files[0])fn(input.files[0]);input.value=''});['dragenter','dragover'].forEach(e=>box.addEventListener(e,x=>{x.preventDefault();box.classList.add('drag')}));['dragleave','drop'].forEach(e=>box.addEventListener(e,x=>{x.preventDefault();box.classList.remove('drag')}));box.addEventListener('drop',e=>e.dataTransfer.files[0]&&fn(e.dataTransfer.files[0]))}
...
function savedDeckId(){try{return sessionStorage.getItem(SESSION_KEY)||''}catch{return''}}
function setCurrentDeck(deck){state.deck=deck;try{sessionStorage.setItem(SESSION_KEY,deck.deckId)}catch{}}
function clearSavedDeck(){try{sessionStorage.removeItem(SESSION_KEY)}catch{}}
function validatePptx(file){if(!/\.pptx$/i.test(file.name))return'请选择 .pptx 文件。';if(file.size>MAX_UPLOAD_BYTES)return`文件超过 ${Math.round(MAX_UPLOAD_BYTES/1024/1024)} MB，未开始检查。`;return''}
function showCheckProgress(file){const el=document.getElementById('check-result');el.innerHTML=`<div class="busy-line"><strong>${esc(file.name)}</strong><span>正在本地处理：解析文件 → 检查风险 → 生成调整步骤</span></div>`;el.classList.add('show')}
async function check(file){const invalid=validatePptx(file);if(invalid){showError('check-result',invalid);return}const card=document.getElementById('check-card'),previous=state.deck;renderActions({});showCheckProgress(file);card.classList.add('spinner');try{const scenario=document.getElementById('scenario').value;const data=await api(`/api/check?filename=${encodeURIComponent(file.name)}&scenario=${scenario}`,{method:'POST',headers:{'Content-Type':'application/vnd.openxmlformats-officedocument.presentationml.presentation'},body:file});setCurrentDeck(data);renderCheck(data);renderActions(data)}catch(e){state.deck=previous;if(previous){renderCheck(previous);renderActions(previous);document.getElementById('check-result').insertAdjacentHTML('afterbegin',`<div class="status blocked"><b>${esc(e.message)}</b></div>`)}else{showError('check-result',e.message)}}finally{card.classList.remove('spinner')}}
function taskMarkup(t){const loc=t.slideIndex?`第 ${t.slideIndex} 页`:'整个文件',count=t.occurrenceCount||1,countText=count>1?`同类 ${count} 处 · `:'',actionLabel={'cleanup-copy':'可生成清理副本','guided-powerpoint':'按步骤人工调整','agent-rebuild':'不建议自动处理','human-decision':'先判断是否处理'}[t.repairMode]||t.repairMode,where=count>1?`${loc}，共 ${count} 处同类命中`:`${loc}`;return `<details class="task"><summary><span></span><span class="task-title"><b>${esc(t.consequence)}</b><small>${esc(countText)}${esc(riskLabels[t.risk]||t.risk)} · ${esc(actionLabel)}</small></span></summary><div class="task-body"><b>在哪里</b><p>${where}；详细对象位置可在中文检查报告中查看。</p><b>希望改成什么样</b><p>${esc(t.target)}</p><b>自己在 PowerPoint 里这样改</b><ol>${(t.steps||[]).map(step=>`<li>${esc(step)}</li>`).join('')}</ol><b>改完这样检查</b><p>以 100% 比例或放映模式查看修改页，再把独立副本拖入第三步复检。</p><div class="task-route"><span><strong>${esc(modeLabels[t.repairMode]||t.repairMode)}</strong><small>${esc(routeDetails[t.repairMode]||'按完整报告中的处理方式继续。')}</small></span></div></div></details>`}
function mergePageTasks(tasks){const merged=new Map();tasks.forEach(task=>{const key=[task.ruleId,task.repairMode,task.risk].join('|'),existing=merged.get(key);if(existing){existing.occurrenceCount+=1;existing.taskIds.push(task.taskId)}else{merged.set(key,{...task,occurrenceCount:1,taskIds:[task.taskId]})}});return [...merged.values()]}
function groupTasks(tasks){const grouped=new Map();tasks.forEach(task=>{const key=task.slideIndex?`slide-${task.slideIndex}`:'whole-file';if(!grouped.has(key))grouped.set(key,{slideIndex:task.slideIndex||null,tasks:[]});grouped.get(key).tasks.push(task)});return [...grouped.values()].map(group=>{group.rawCount=group.tasks.length;group.tasks=mergePageTasks(group.tasks);group.tasks.sort((a,b)=>(riskRanks[a.risk]??9)-(riskRanks[b.risk]??9)||String(a.ruleId).localeCompare(String(b.ruleId)));group.risk=Math.min(...group.tasks.map(task=>riskRanks[task.risk]??9));return group}).sort((a,b)=>a.risk-b.risk||(a.slideIndex===null?1:b.slideIndex===null?-1:a.slideIndex-b.slideIndex))}
function pageGroupMarkup(group){const loc=group.slideIndex?`第 ${group.slideIndex} 页`:'整个文件',summary=group.rawCount>group.tasks.length?`${group.tasks.length} 类 · 共 ${group.rawCount} 处（已合并重复）`:`${group.tasks.length} 项`;return `<section class="page-group"><div class="page-head"><strong>${loc}</strong><span>${summary}</span></div>${group.tasks.map(taskMarkup).join('')}</section>`}
function renderCheck(d,restored=false){const r=d.readiness||{},cls=r.status||'review',label={ready:'可以交付',review:'发送前再看一眼',blocked:'先处理再发送'}[cls]||'需要检查',tasks=d.tasks||[],actionPages=new Set(tasks.filter(t=>Number.isInteger(t.slideIndex)&&(t.steps||[]).length).map(t=>t.slideIndex)).size,cleanupCount=(d.cleanupOperations||[]).length,humanCount=d.modeCounts['human-decision']||0,groups=groupTasks(tasks),visibleTasks=groups.reduce((sum,group)=>sum+group.tasks.length,0),collapsed=tasks.length-visibleTasks,fileName=d.file&&d.file.name?d.file.name:'当前 PPT',taskSummary=collapsed>0?`${tasks.length} 处命中，已合并为 ${visibleTasks} 个处理任务；完整报告保留每一处证据。`:`${tasks.length} 项任务，已按风险和页面整理；辅助分数请查看完整报告。`,recovery=restored?'<div style="margin:0 0 10px;padding:9px 11px;border-left:3px solid var(--green);background:#edf7f3;color:var(--muted);font-size:12px">已恢复当前标签页的本地会话。这里只记住会话编号；原 PPT 仍只存在于本次运行的临时目录，关闭交付台后删除。</div>':'';document.getElementById('check-result').innerHTML=`${recovery}<div class="status ${cls}"><div><b>${label}</b><div style="overflow-wrap:anywhere">${esc(fileName)} · ${taskSummary}</div></div></div><div class="metrics"><div class="metric"><span>待处理页面</span><strong>${actionPages}</strong></div><div class="metric"><span>可安全清理</span><strong>${cleanupCount}</strong></div><div class="metric"><span>需要判断</span><strong>${humanCount}</strong></div></div><div class="tasks">${groups.length?groups.map(pageGroupMarkup).join(''):'<div class="status"><b>没有需要处理的任务</b></div>'}</div>${downloads(d.downloads)}`;document.getElementById('check-result').classList.add('show')}
function renderActions(d){const box=document.getElementById('cleanup-checks'),ops=d.cleanupOperations||[],ready=Boolean(d.deckId);box.innerHTML=ops.length?ops.map(op=>`<label><input type="checkbox" value="${op}">${opLabels[op]}</label>`).join(''):`<span style="color:var(--muted)">${ready?'没有可由 PPTLint 自动清理的项目。':'等待检查结果。'}</span>`;document.getElementById('cleanup-btn').disabled=!ops.length;document.getElementById('copy-btn').disabled=!ready}
...
async function copyBrief(){if(!state.deck)return;try{await navigator.clipboard.writeText(state.deck.agentBrief);document.getElementById('copy-btn').textContent='已复制完整任务'}catch{showMessage('浏览器未允许写入剪贴板，请下载 Agent 修复简报。',false,true)}}
...
async function restoreSession(){const deckId=savedDeckId();if(!deckId)return;try{const data=await api(`/api/session?deckId=${encodeURIComponent(deckId)}`);setCurrentDeck(data);renderCheck(data,true);renderActions(data)}catch{clearSavedDeck()}}
wireDrop('check-drop',check);wireDrop('verify-drop',verify);document.getElementById('cleanup-btn').addEventListener('click',cleanup);document.getElementById('copy-btn').addEventListener('click',copyBrief);
restoreSession();
</script></body></html>'''
    return (
        template.replace("__TOKEN__", json.dumps(token))
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/session":
            if not self._authorized():
                return
            deck_id = parse_qs(parsed.query).get("deckId", [""])[0]
            record = self.server.session.decks.get(deck_id)
            if record is None:
                self._json({"error": "当前 PPT 会话已结束。"}, HTTPStatus.NOT_FOUND)
                return
    ...
    def _verify(self, parsed) -> None:
        ...
        if passed:
            ...
            svg_path.write_text(f'''<svg xmlns="http://www.w3.org/2000/svg" width="320" height="72" role="img" aria-label="PPTLint Verified"><a href="{verification_path.name}"><rect width="320" height="72" rx="14" fill="#0a1628"/><circle cx="38" cy="36" r="18" fill="#0d8b63"/><path d="M29 36l6 6 12-14" fill="none" stroke="white" stroke-width="4"/><text x="68" y="31" fill="white" font-family="PingFang SC, Microsoft YaHei, sans-serif" font-size="13" font-weight="700">PPTLINT</text><text x="68" y="51" fill="#9ee4cc" font-family="PingFang SC, Microsoft YaHei, sans-serif" font-size="20" font-weight="800">VERIFIED</text></a></svg>''', encoding="utf-8")
