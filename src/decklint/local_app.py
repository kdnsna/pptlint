...
<script>
const TOKEN=__TOKEN__;const MAX_UPLOAD_BYTES=__MAX_UPLOAD_BYTES__;const state={deck:null};
...
function renderCheck(d){const r=d.readiness||{},cls=r.status||'review',label={ready:'可以交付',review:'发送前再看一眼',blocked:'先处理再发送'}[cls]||'需要检查',tasks=d.tasks||[],actionPages=new Set(tasks.filter(t=>Number.isInteger(t.slideIndex)&&(t.steps||[]).length).map(t=>t.slideIndex)).size,cleanupCount=(d.cleanupOperations||[]).length,humanCount=d.modeCounts['human-decision']||0,groups=groupTasks(tasks);document.getElementById('check-result').innerHTML=`<div class="status ${cls}"><div><b>${label}</b><div>${tasks.length} 项任务，已按风险和页面整理；辅助分数请查看完整报告。</div></div></div><div class="metrics"><div class="metric"><span>待处理页面</span><strong>${actionPages}</strong></div><div class="metric"><span>可安全清理</span><strong>${cleanupCount}</strong></div><div class="metric"><span>需要判断</span><strong>${humanCount}</strong></div></div><div class="tasks">${groups.length?groups.map(pageGroupMarkup).join(''):'<div class="status"><b>没有需要处理的任务</b></div>'}</div>${downloads(d.downloads)}`;document.getElementById('check-result').classList.add('show')}
