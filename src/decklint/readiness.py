...
ZH_FIX_STEPS = {
    ...
    "privacy.external-relationship": ["在 PowerPoint 的“文件 > 信息”中检查链接和外部内容；WPS 的入口位置可能不同。", "先确认链接承载的是图表数据、图片还是跳转地址，再选择嵌入、断开或保留；不要直接删除后猜测丢失内容。", "另存交付副本，断网重新打开并逐项核对现有图表和链接，再运行 PPTLint。"],
    ...
    "readability.off-canvas-text": ["在报告所示页面用选择窗格定位越界对象，先判断它属于普通文本、表格标签还是截图说明。", "普通对象移回安全边距；表格优先调整列宽和单元格内边距；截图保持宽高比、裁掉无关空白后再对齐说明框，不要只缩小字号。", "以 100% 比例或放映模式检查边缘，逐项核对文字、机构名和数字，另存副本后再运行 PPTLint。"],
    "readability.text-overlap": ["在报告所示页面用选择窗格定位相互遮挡的文本框、表格或截图说明，并确认各对象对应的内容。", "先调整文本框高度、段落间距、列宽或说明框位置，再使用“对齐 / 横向分布 / 纵向分布”统一边线；空间不足时重排，不用极小字号硬塞。", "以 100% 比例检查遮挡和留白，逐项核对关键信息没有被裁切或拆散，另存副本后再运行 PPTLint。"],
    ...
    "readability.small-font": ["切换到幻灯片浏览视图，先确认小字属于正文、表格、图注还是截图标注，并与相邻页面同层级文字比较。", "正文优先增加文本框空间或改为两栏；表格调整列宽和内边距；截图标注先裁掉无关空白再重新对齐，不用自动缩排硬塞。", "按实际观看距离播放一遍，逐项确认数字和条件说明仍可辨认，另存副本后再运行 PPTLint。"],
    ...
    "readability.dense-text": ["先确定观众看完这一页必须记住的唯一结论，并核对所有事实、数字和限定条件都需要保留。", "不改业务含义时，使用标题、分组、两栏、表格内边距和留白建立层级；若仍无法达到最小字号，由用户决定是否拆页。", "以放映模式计时阅读并逐项核对数字、排名和结论，另存副本后再运行 PPTLint。"],
