...
def build_report(
    ...
    *,
    ...
    return {
        ...
        "summary": summary,
        "findings": [finding.to_dict(deck_width=deck.width, deck_height=deck.height) for finding in findings],
        "slides": [
            {
                "index": slide.index,
                "title": slide.title,
...
def _render_html(report: dict[str, object]) -> str:
    ...
    for slide in slides:
        ...
        for finding in slide_findings:
            ...
            finding_items += (
                ...
                f'<code>{_escape(finding["rule_id"])}</code><strong>{_escape(finding["message"])}</strong>'
    ...
    deck_items = "".join(
        f'<li class="finding severity-{_escape(f["severity"])}"><code>{_escape(f["rule_id"])}</code>'
        f'<strong>{_escape(f["message"])}</strong><p>{_escape(f["evidence"])}</p><small>{_escape(f["remediation"])}</small></li>'
